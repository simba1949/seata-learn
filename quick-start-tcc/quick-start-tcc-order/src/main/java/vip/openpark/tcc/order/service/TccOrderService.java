package vip.openpark.tcc.order.service;
...
import vip.openpark.tcc.order.domain.OrderDO;
import vip.openpark.tcc.order.mapper.OrderDOMapper;
...
public class TccOrderService {
	...
	private final TccStockFacade tccStockFacade;
	private final TccAccountFacade tccAccountFacade;

	@GlobalTransactional(name = "tcc-order-stock-account", rollbackFor = Exception.class)
	public void createOrder(Long userId, Long productId, Integer quantity) {
		...
		log.info("开始扣减库存");
		Boolean deduct = tccStockFacade.deduct(xid, productId, quantity);
		...
		log.info("开始扣减余额");
		Boolean deduct1 = tccAccountFacade.deduct(xid, userId, BigDecimal.TEN);
	...
	private OrderDO buildOrder(Long userId, Long productId, Integer quantity, String code) {
		...
		orderDO.setStatus((byte) 0); // 订单状态，0表示创建中，1表示已完结
